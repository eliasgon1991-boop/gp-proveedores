...
const ENDPOINT_DATOS = "https://egonzalezm.app.n8n.cloud/webhook/gp-proveedores";
const ENDPOINT_BACKEND = ""; // ej: "https://gp-backend.up.railway.app"
...
export default function GPProveedoresFeed() {
  ...
  const [feed, setFeed] = useState(SEMILLA);
  ...
  const pasar = useCallback(
    ...
    [actual, saliendo, panel]
  );

  const volver = useCallback(() => {
    if (saliendo || panel) return;
  ...
  }, [indice, saliendo, panel, historial]);
  ...
  }, [pasar, volver, panel, gustada, actual]);
  ...
  const racha = (
    <button
      onClick={() => avisar("Llevas 7 días seguidos postulando en Mercado Público. ¡No cortes la racha! 🔥")}
      title="Días seguidos postulando en Mercado Público"
      aria-label="Racha: 7 días seguidos postulando"
      style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 12, background: GOLD_BG, border: `1px solid ${GOLD_DEEP}`, cursor: "pointer" }}>
      <span style={{ fontSize: 15, animation: "brasa 1.8s ease-in-out infinite" }}>🔥</span>
      <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, fontWeight: 700, color: "#ffd479" }}>7</span>
      <span style={{ fontSize: 9, letterSpacing: "0.05em", color: "#f3ca7c", textTransform: "uppercase", fontWeight: 700 }}>días de racha postulando</span>
    </button>
  );
  ...
  const rachaCard = (
    <button
      onClick={() => avisar("Llevas 7 días seguidos postulando. ¡No cortes la racha! ⚡")}
      aria-label="Racha: 7 días seguidos postulando"
      style={{ textAlign: "left", width: "100%", background: GOLD_BG, border: `1px solid ${GOLD_DEEP}55`, borderRadius: 16, padding: "14px 14px 12px", cursor: "pointer" }}>
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
        <span style={{ width: 34, height: 34, borderRadius: 10, background: GOLD, color: "#1a1206", display: "grid", placeItems: "center" }}>
          <Icono name="bolt" size={18} fill />
        </span>
        <span>
          <span style={{ display: "block", fontFamily: "'Manrope', sans-serif", fontWeight: 800, fontSize: 16, color: "#ffd479", lineHeight: 1 }}>7 días</span>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
        {diasRacha.map((l, i) => (
          <span key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <span style={{ width: 15, height: 15, borderRadius: "50%", background: GOLD, opacity: i === 6 ? 1 : 0.55, border: i === 6 ? `2px solid #ffd479` : "none", boxShadow: i === 6 ? "0 0 8px rgba(255,212,121,.5)" : "none" }} />
            <span style={{ fontSize: 8.5, color: MUTED, fontFamily: "'IBM Plex Mono', monospace" }}>{l}</span>
          </span>
        ))}
      </div>
      <div style={{ fontSize: 10.5, color: "#c9c6bf", lineHeight: 1.5 }}>Vuelve mañana antes de las 12:00 para mantenerla.</div>
    </button>
  );
  ...
  const tarjeta = actual ? (
    <article
      ...
      style={{
        ...
        border: movil ? "none" : `1px solid ${actual.similar ? BORDE : GOLD_DEEP}`,
        padding: movil ? "72px 22px 128px" : "30px 32px",
        display: "flex", flexDirection: "column",
        cursor: "grab",
        boxShadow: movil ? "none" : "0 22px 60px rgba(0,0,0,.5)",
        zIndex: 2,
        ...cardStyle,
      }}
    >
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <span style={{ marginLeft: "auto", fontSize: 12, fontFamily: "'IBM Plex Mono', monospace", color: MUTED }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11.5, marginBottom: 6, fontFamily: "'Manrope', sans-serif" }}>
          <span style={{ color: MUTED }}>Calce con tu pyme</span>
          <span style={{ color: actual.similar ? "#c9c6bf" : GOLD, fontWeight: 700, fontFamily: "'IBM Plex Mono', monospace" }}>{actual.match}%</span>
        </div>
        <div style={{ height: 5, borderRadius: 999, background: CARD2 }}>
          <div style={{ width: `${actual.match}%`, height: "100%", borderRadius: 999, background: actual.similar ? "#4a4a55" : `linear-gradient(90deg, ${GOLD_DEEP}, ${GOLD})` }} />
        </div>
        <div style={{ fontSize: 12.5, color: "#c9c6bf", marginTop: 8, lineHeight: 1.5, fontFamily: "'Manrope', sans-serif" }}>{actual.razon}</div>
      </div>

      {/* Insight del comprador */}
      {actual.insight && (
        <div style={{ background: GOLD_BG, border: `1px solid #3b2b0f`, borderRadius: 14, padding: "11px 13px", marginBottom: 14 }}>
          <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: "0.1em", color: GOLD_DEEP, textTransform: "uppercase", marginBottom: 4, fontFamily: "'IBM Plex Mono', monospace" }}>✦ Dato del comprador</div>
          <div style={{ fontSize: 12.5, lineHeight: 1.55, color: "#f3ca7c", fontFamily: "'Manrope', sans-serif" }}>{actual.insight}</div>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
        <button onClick={() => setPanel("ficha")}
          style={{ flex: 2.2, padding: "15px 10px", borderRadius: 14, border: `1.5px solid ${GOLD}`, background: GOLD, color: "#1a1206", fontWeight: 800, fontSize: 15, cursor: "pointer", fontFamily: "'Manrope', sans-serif" }}>
          Ver ficha completa
        </button>
    ...
    <div style={{ textAlign: "center", padding: 32, maxWidth: 340, zIndex: 2 }}>
      ...
      {filtro === "urgentes" && (
        <button onClick={() => cambiarFiltro("todas")} style={{ ...btn({ fondo: GOLD, borde: GOLD, color: "#1a1206", peso: 800 }), width: "auto", padding: "12px 22px" }}>
          Volver al reparto
        </button>
      )}
    </div>
      ...
      {!movil && (
        <>
          ...
          <button onClick={() => pasar("up")} aria-label="Pasar a la siguiente"
            style={{ width: 56, height: 56, borderRadius: "50%", background: PAPER, border: `1px solid ${PAPER}`, color: "#111114", display: "grid", placeItems: "center", cursor: "pointer" }}>
  ...
  const peek = (f, arriba) => f && !movil && (
    <div aria-hidden="true" style={{
      ...
      background: "#111114", border: `1px solid #1d1d22`, borderRadius: 18,
  ...
  return (
    <div style={{
      ...
      {/* ── Sidebar (escritorio) ── */}
      {!movil && (
        <aside style={{ borderRight: `1px solid #1b1b20`, padding: "26px 16px 18px", display: "flex", flexDirection: "column", gap: 6, background: "#111114", overflowY: "auto" }}>
          ...
          {NAVS.map((n) => (
            <button key={n.id} onClick={n.onClick}
              style={{
                ...
                background: navActiva === n.id ? "#1d1d22" : "transparent",
                border: `1px solid ${navActiva === n.id ? "#26262d" : "transparent"}`,
              ...
              {n.badge && n.badge !== "0" && (
                <span style={{ minWidth: 22, height: 22, borderRadius: 999, background: navActiva === n.id ? GOLD : "#26262d", color: navActiva === n.id ? "#1a1206" : MUTED, display: "grid", placeItems: "center", fontSize: 11, fontWeight: 700, fontFamily: "'IBM Plex Mono', monospace", padding: "0 7px" }}>{n.badge}</span>
              )}
            </button>
          ))}
          <div style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: 12, padding: "12px 4px 0" }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", paddingTop: 12, borderTop: "1px solid #1b1b20" }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#26262d", color: GOLD, display: "grid", placeItems: "center", fontWeight: 800, fontSize: 12.5 }}>AR</div>
              <div>
      ...
      <main onWheel={onWheel} style={movil
        ...
        {movil && (
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 9, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "linear-gradient(180deg, rgba(17,17,20,.92), transparent)" }}>
            <div>
        ...
        {!movil && actual && (
          <div style={{ position: "absolute", bottom: 14, left: "50%", transform: "translateX(-50%)", fontSize: 11.5, fontFamily: "'IBM Plex Mono', monospace", color: MUTED, zIndex: 4, background: "#1b1b20ee", border: `1px solid ${BORDE}`, borderRadius: 999, padding: "8px 18px", whiteSpace: "nowrap" }}>
        ...
        {panel === "ficha" && actual && (
          <Hoja titulo={`Ficha · ID ${actual.id}`} cerrar={() => setPanel(null)} movil={movil}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => window.open(urlFicha(actual), "_blank")} style={btn({ fondo: GOLD, borde: GOLD, color: "#1a1206", peso: 800 })}>Abrir en Mercado Público ↗</button>
            </div>
          </Hoja>
        )}
        ...
        {panel === "perfil" && (
          <Hoja titulo="Mi pyme: ¿qué vendes?" cerrar={() => setPanel(null)} movil={movil}>
            ...
            <button onClick={guardarPerfil} style={btn({ fondo: GOLD, borde: GOLD, color: "#1a1206", peso: 800 })}>Guardar y reordenar mi reparto</button>
          </Hoja>
        )}
          ...
          <Hoja titulo="Tu carro de ventas" cerrar={() => setPanel(null)} movil={movil}>
            {guardadas.length === 0 ? (
              ...
              <>
                ...
                <button onClick={() => { setPanel(null); avisar("A postular: cada ficha se abre con ↗"); }} style={{ ...btn({ fondo: GOLD, borde: GOLD, color: "#1a1206", peso: 800 }), width: "100%", marginTop: 6 }}>
                  ...
                  <div key={g.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 13, border: `1px solid ${g.resultado === "adjudicada" ? GOLD_DEEP : BORDE}`, background: g.resultado === "adjudicada" ? GOLD_BG : CARD2, marginBottom: 8 }}>
                    ...
                    {g.resultado === "adjudicada" && (
                      <button onClick={() => publicarAdjudicacion(g)}
                        style={{ background: GOLD, border: `1.5px solid ${GOLD}`, borderRadius: 9, color: "#1a1206", cursor: "pointer", fontSize: 11, padding: "6px 9px", fontWeight: 800 }}>🏆 Publicar</button>
                    )}
                  ...
                  <div style={{ background: CARD2, border: `1px solid ${BORDE}`, borderRadius: 13, padding: "12px 14px", marginBottom: 12 }}>
                    ...
                    {mes.ventas.map((v) => (
                      <div key={v.mecanismo} style={{ marginBottom: 8 }}>
                        ...
                        <div style={{ height: 6, borderRadius: 999, background: "#26262d" }}>
        ...
        {toast && (
          <div style={{ position: "absolute", bottom: movil ? 118 : 44, left: "50%", transform: "translateX(-50%)", background: PAPER, color: "#111114", fontSize: 13, fontWeight: 700, padding: "9px 16px", borderRadius: 999, boxShadow: "0 6px 20px rgba(0,0,0,.4)", zIndex: 40, maxWidth: "90%", textAlign: "center" }}>
            {toast}
          </div>
        )}
      </main>
    ...
    <div onClick={cerrar} style={{ position: "absolute", inset: 0, background: "rgba(10,10,12,.66)", display: "flex", alignItems: movil ? "flex-end" : "center", justifyContent: "center", zIndex: 30 }}>
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          ...
          <button onClick={cerrar} aria-label="Cerrar" style={{ background: "#1b1b20", border: "1px solid #2c2c34", borderRadius: 9, color: "#c9c6bf", fontSize: 14, cursor: "pointer", padding: "5px 9px" }}>✕</button>
        </div>
        {children}
      </div>
    </div>
...
function BotonHoja({ onClick, icono, texto, sub, dorado }) {
  return (
    <button onClick={onClick}
      style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, textAlign: "left", background: dorado ? "#1a1206" : "transparent", border: `1.5px solid ${dorado ? "#e9b44c" : "#2c2c34"}`, borderRadius: 14, padding: "12px 14px", marginBottom: 8, cursor: "pointer", color: "#f2efe9" }}>
      ...
      <span>
        ...
        <span style={{ display: "block", fontSize: 11.5, color: "#8b8b93", marginTop: 2 }}>{sub}</span>
      </span>
    </button>
  );
}
...
function Dato({ etiqueta, valor, urgente, tono, grande }) {
  const color = urgente ? "#ff8a3d" : tono === "verde" ? "#5ad48e" : tono === "oro" ? "#e9b44c" : "#f2efe9";
  return (
    <div style={{ flex: 1, background: "#1b1b20", border: `1px solid ${urgente ? "#ff6b5a66" : "#2c2c34"}`, borderRadius: 13, padding: grande ? "12px 14px" : "10px 12px" }}>
      <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: "0.1em", color: "#8b8b93", fontFamily: "'IBM Plex Mono', monospace" }}>{etiqueta}</div>
